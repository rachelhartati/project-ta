...
<body>
                                ...
                                <td>
                                    <div class="action">
                                        ...
                                        <button class="btn btn-delete" data-bs-toggle="modal"
                                            data-bs-target="#modalHapus" data-id="{{ $users->id }}">
                                            Hapus
                                        </button>
    ...
    @include('user.modal.modalTambah')
    @include('user.modal.modalEdit')
    @include('user.modal.modalHapus')
    ...
    <script>
        document.querySelectorAll('.btn-edit').forEach(button => {
            button.addEventListener('click', function () {
                ...
                // Set action form
                document.querySelector('#modalEdit form').action = `/user/${id}`;
                ...
                fetch(`/user/${id}/data`)
                    ...
                    .then(data => {
                        ...
                        // Isi input biasa
                        document.querySelector('#modalEdit [name="nama_lengkap"]').value = user
                            .nama_lengkap;
                        document.querySelector('#modalEdit [name="alamat"]').value = user.alamat;
                        document.querySelector('#modalEdit [name="no_tlp"]').value = user.no_tlp;
                        document.querySelector('#modalEdit [name="status"]').value = user.status;

                        // Isi dropdown agent
                        const agentSelect = document.querySelector('#modalEdit [name="agent_id"]');
                        agentSelect.innerHTML = '<option value="" disabled>Pilih agent</option>';
                        data.agents.forEach(agent => {
                            agentSelect.innerHTML +=
                                `<option value="${agent.id}" ${agent.id == user.agent_id ? 'selected' : ''}>${agent.nama}</option>`;
                        });

                        // Isi dropdown role
                        const roleSelect = document.querySelector('#modalEdit [name="role"]');
                        if (roleSelect) {
                            roleSelect.innerHTML = '<option value="" disabled>Pilih role</option>';
                            data.roles.forEach(role => {
                                const userRole = user.roles[0] ? user.roles[0].name : '';
    ...
    console.log('id:', id);
    document.querySelector('#formHapus').action = `/user/${id}`;
});
    </script>
